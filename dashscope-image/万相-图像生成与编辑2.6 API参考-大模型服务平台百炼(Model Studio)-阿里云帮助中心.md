# 万相-图像生成与编辑2.6 API参考-大模型服务平台百炼(Model Studio)-阿里云帮助中心
...
北京、新加坡和弗吉尼亚地域拥有独立的 API Key 与请求地址，不可混用，跨地域调用将导致鉴权失败或服务报错，详情请参见 [选择部署模式](https://help.aliyun.com/zh/model-studio/regions/)。
...
仅当`parameters.enable_interleave=true` 时，必须将该字段设为`enable`。
...
当`enable_interleave=false` 时（图像编辑），必须输入1~4张图像。
...
max_images影响费用。费用 = 单价 × 成功生成的图片张数，请在调用前确认 [模型价格](https://help.aliyun.com/zh/model-studio/model-pricing#e2540d71a2utl)。
...
控制返回结果是否为流式输出。在图像混排模式（即`enable_interleave = true`）下，必须设置为`true`。
...
传输结束时：仅在最后一个 JSON 结构体中返回`"finish_reason":"stop"`，表示流式请求已全部结束，应停止接收。
...
轮询建议：图像生成过程较长，建议采用轮询机制，并设置合理的查询间隔（如 10 秒）来获取结果。
...
请确保 DashScope Python SDK 版本不低于`1.25.8`，再运行以下代码。更新请参考 [安装SDK](https://help.aliyun.com/zh/model-studio/install-sdk)。
